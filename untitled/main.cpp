...
public:
    ...
    Band(): historyOfTransmissionState(historySizeForTransmissionVector, 0){}
...
void fillHistoryOfTransmissionState(vector <Band> &PU, vector<unsigned int> &freqArr){
    for(int i=0; i< freqArr.size(); i++){
        if(freqArr[i] >0){
            PU[i].historyOfTransmissionState.push_back(1);
        }else{
            PU[i].historyOfTransmissionState.push_back(0);
        }

        PU[i].historyOfTransmissionState.pop_front();
    }
}
